...
fn main() {
    ...
    window.on_package_clicked(move |id| {
        ...
        if let Some(window) = window_weak.upgrade() {
            ...
            if let Some(pkg) = state.config.packages.iter()
                ...
                // Build items list for Slint
                let items: Vec<ItemData> = pkg.items.iter().map(|item| {
                    ItemData {
                        name: item.name.clone().into(),
                        path: item.path.clone().into(),
                        is_folder: matches!(item.item_type,
                            config::ItemType::Folder),
                        exists: filesystem::path_exists(&item.path),
                    }
                }).collect();

                window.set_expanded_items(
    ...
    // Setup always-on-top after window is visible
    // Note: Get HWND here using platform-specific code
    // See SKILL.md Section 7 for details

    // Run the event loop
    slint::run_event_loop().unwrap();
}

// Refresh the packages model from current state
fn refresh_packages(window: &MainWindow, state: &Arc<Mutex<state::AppState>>) {
    let state = state.lock().unwrap();

    let packages: Vec<PackageData> = state.config.packages.iter().map(|pkg| {
        PackageData {
            id: pkg.id.clone().into(),
            name: pkg.name.clone().into(),
            color: pkg.color.clone().into(),
            item_count: pkg.items.len() as i32,
            items: ModelRc::new(VecModel::from(
                pkg.items.iter().map(|item| ItemData {
                    name: item.name.clone().into(),
                    path: item.path.clone().into(),
                    is_folder: matches!(item.item_type, config::ItemType::Folder),
                    exists: filesystem::path_exists(&item.path),
                }).collect::<Vec<_>>()
            )),
        }
    ...
    window.set_packages(ModelRc::new(VecModel::from(packages)));
}
```

**Commit:**
```
feat: rust backend — filesystem, window setup, main loop with package interactions
```

---

## ➕ Phase 5 — Add Items To Packages

Users need to add folders/files to packages. Two ways:

### Method 1 — Right-click context menu on package

When user right-clicks a package card — show a context menu:
- "Add from Desktop" — shows desktop items to pick from
- "Rename package"
- "Change color"
- "Delete package"

### Method 2 — Desktop item picker dialog

A simple Slint dialog that lists all current desktop items.
User clicks one to add it to the package.

```slint
// ui/picker.slint
export struct DesktopItem {
    name: string,
    path: string,
    is-folder: bool,
}

export component DesktopPicker inherits Dialog {
    title: "Add to Package";
    width: 320px;
    height: 400px;

    in property <[DesktopItem]> desktop-items: [];
    callback item-selected(string);  // path
    ...
    VerticalLayout {
        ...
        Text {
            text: "Choose from Desktop:";
            color: #F0EDE8;
            font-size: 14px;
            font-weight: 600;
        }

        for item in desktop-items : HorizontalLayout {
            spacing: 10px;
            padding: 8px;
            ...
            TouchArea {
                clicked => { root.item-selected(item.path); }
            }
        }
    }
}
...
**Commit:**
```
feat: desktop item picker — add folders and files to packages
```

---

## 🎯 Phase 6 — Positioning and Dragging

### Pill always on right edge
On startup — position the pill on the right edge of the screen:

```rust
// In main.rs — after window.show()
let screen_width = 1920; // TODO: get actual screen width via Windows API
let screen_height = 1080;

let pill_height = state.config.packages.len() as i32 * 58 + 24;
let x = screen_width - 80;
let y = (screen_height - pill_height) / 2;

window.window().set_position(slint::PhysicalPosition::new(x, y));
```

### Make pill draggable
The pill must be draggable to any screen position.
Track mouse drag on the pill background and update window position.

```slint
// In pill.slint — add drag support
drag-area := TouchArea {
    // Only allow drag on the pill background, not on cards
    mouse-cursor: move;

    moved => {
        // Send new position to Rust
        root.drag-moved(self.mouse-x, self.mouse-y);
    }
}
```
...
**Commit:**
```
feat: pill positioning on right edge, drag to reposition, save position
```
...
### Startup animation
When Clutch starts — the pill slides in from the right edge:
```slint
// Pill starts off-screen, slides in
x: started ? parent.width - self.width - 4px : parent.width + 10px;

animate x {
    duration: 400ms;
    easing: cubic-bezier(0.16, 1, 0.3, 1);
    delay: 200ms;  // slight delay after launch
}
```

### Missing item visual
Items whose paths no longer exist show with:
- Strikethrough text
- Muted color
- Small red ✕ indicator
- Right-click → "Remove missing item"

### System tray
Add a system tray icon so users can:
- Show/hide Clutch
- Open settings
- Quit
...
pub fn create_tray() -> TrayIcon {
    let menu = Menu::new();
    menu.append(&MenuItem::new("Show Clutch", true, None));
...
Add registry entry so Clutch starts with Windows:
```rust
// Add to HKCU\Software\Microsoft\Windows\CurrentVersion\Run
fn enable_autostart() {
    // Use windows-rs registry API
}
```

**Commit:**
```
feat: startup animation, missing item detection, system tray, autostart
```
...
Clutch is a lightweight desktop overlay that groups your scattered
files and folders into clean visual packages. Nothing moves.
Nothing gets deleted. Just a beautiful pill-shaped dock that
keeps everything organized and one hover away.

## How It Works

1. Launch Clutch — a pill appears on your screen edge
2. Click "+" to create a package
3. Add folders and files from your desktop to the package
4. Hover the package card — see everything inside
5. Click any item — it opens directly

## Features

- Floating pill dock — always visible, never intrusive
- Colored packages — group by project, category, whatever
- Spring animations — smooth, Apple-quality feel
- Zero RAM overhead — under 5MB idle
- Nothing moves — your files stay exactly where they are
- Drag to reposition — put the pill anywhere on screen

## Tech

Built with Rust + Slint — GPU-accelerated, native performance.
No Electron. No browser. No bloat.
...
**Commit:**
```
docs: readme with features, how it works, build instructions
```

---

## 🔀 Complete Git Commit Order

```
feat: clutch project setup — rust, slint, cargo configuration
feat: config structs, app state, package and item management
feat: slint UI components — card with spring animation, panel, pill, window
feat: rust backend — filesystem, window setup, main event loop
feat: desktop item picker — add folders and files to packages
feat: pill positioning on right edge, drag to reposition
feat: startup animation, missing items, system tray, autostart
docs: readme with features, build instructions
```

> One commit per phase. Push after every commit. No exceptions.

---

## 🚫 Agent Rules

1. Read SKILL.md completely before reading this prompt
2. Save this as `BUILD.md` in the project root before writing code
3. Build in exact phase order — UI before backend wiring
4. The pill must be transparent and always-on-top — non-negotiable
5. Use spring easing `cubic-bezier(0.34, 1.56, 0.64, 1)` for all hover animations
6. Files are NEVER moved or deleted — Clutch is read-only for the filesystem
7. Config saves after every user action — no data loss
8. Test `cargo run` after every phase before committing
9. One commit per phase — push immediately after

---

## ✅ Definition of Done

Clutch is complete when:
- Pill appears on screen right edge on launch with slide-in animation
- Cards animate with spring physics on hover — exactly like Apple dock
- Clicking a card opens/closes the expanded panel with smooth animation
- Expanded panel shows grouped items with folder icons
- Clicking an item opens it in Windows Explorer or default app
- Adding items from desktop works via the picker
- Pill is draggable — position saves between sessions
- System tray icon with show/hide and quit
- App idles under 5MB RAM
- No console window on Windows release build

---

*Clutch — Your desktop. Finally organized.*
*Built with Rust. Smooth like Apple. Light like nothing.*
